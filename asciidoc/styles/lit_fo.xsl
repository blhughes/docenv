...
<!--General page layout parameters-->
...
<!--Render selected inline elements within titles and the TOC.-->
<xsl:template
    match="filename|sgmltag|userinput|varname|code|application"
    mode="no.anchor.mode">
  <xsl:apply-templates select="." />
</xsl:template>

<!--component.title: Customize the title page-->
<xsl:template name="component.title">
  ...
  <fo:block keep-with-next.within-column="always"
    ...
    <fo:table table-layout="fixed" padding-bottom="0.2in">
      <fo:table-column column-number="1" column-width="4.5in"/>
      <fo:table-column column-number="2" column-width="2.25in"/>
      <fo:table-body>
        <fo:table-row>

          <!--First column displays the title.-->
          <fo:table-cell>
          ...
          <!--Second column displays the logo image.-->
          <fo:table-cell>
            <fo:block>
...
<!--revhistory: Generate a timestamp block-->
<xsl:template match="revhistory" mode="titlepage.mode">
  <xsl:call-template name="timestamp.block"/>
</xsl:template>

<!--timestamp.block: Display a timestamp as YYYY-MM-DD hh:mm-->
<xsl:template name="timestamp.block">
